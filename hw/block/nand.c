...
static void nand_command(NANDFlashState *s)
{
    ...
    case NAND_CMD_READID:
        ...
        if (s->addr == 0x20) {
#if 1
           nand_pushio_byte(s, 0x4f);  // O
           nand_pushio_byte(s, 0x4e);  // N
           nand_pushio_byte(s, 0x46);  // F 
           nand_pushio_byte(s, 0x49);  // I 
#else
        ptr = strtoul(s->param, &bufHex, 16);
        /* Load first 4 bytes of the NAND parameter page...ONFI signature */
        for (i=0; i < 0x4; i++) { 
           //sprintf(bufHex, "%X", (uint8_t)s->param[i]);
           nand_pushio_byte(s, ptr);
           ptr = strtoul(bufHex, &bufHex, 16);
        }
#endif
        } else { 
        nand_pushio_byte(s, s->manf_id);
        nand_pushio_byte(s, s->chip_id);
        nand_pushio_byte(s, 0x90); /* Don't-care byte (often 0xa5) */
        ...
        nand_pushio_byte(s, 0x44); /* Multi-plane Information */
        }
        break;
    ...
    case NAND_CMD_READPARAMPAGE:
        ...
        for (i=0; i < 3; i++) {
           ptr = strtoul(s->param, &bufHex, 16);
           /* Load 256 bytes of the NAND parameter page, repeat up to 3 times on NAND device */
           for (j=0; j < 0x100; j++) { 
//             sprintf(bufHex, "%X", (uint8_t)s->param[j]);
             nand_pushio_byte(s, ptr);
             ptr = strtoul(bufHex, &bufHex, 16);
           }
        }
...
static void nand_realize(DeviceState *dev, Error **errp)
{
    ...
    s->ioaddr = s->io;
DB_PRINT("%s\n size=%d\n pages=%d\n,page_shift=%d\n oob_shift=%d\n buswidth=%d\n chipid=%x\n pagesize=%d\n bdrv=%d\n storage=%lx\n mem_oob=%d\n" , __func__,s->size, s->pages, s->page_shift,s->oob_shift, s->buswidth, s->chip_id, pagesize, (uint)blk_getlength(s->blk), (int long)s->storage, s->mem_oob);
}

static Property nand_properties[] = {
    DEFINE_PROP_UINT8("manufacturer_id", NANDFlashState, manf_id, 0),
    DEFINE_PROP_UINT8("chip_id", NANDFlashState, chip_id, 0),
//    DEFINE_PROP_ARRAY("param_page", NANDFlashState, num_param, param, qdev_prop_uint32, uint32_t),
    DEFINE_PROP_STRING("param_page", NANDFlashState, param),
...
void nand_setio(DeviceState *dev, uint32_t value)
{
    int i;
    NANDFlashState *s = NAND(dev);
DB_PRINT("cle=%x ale=%x ce=%x wp=%x gnd=%x ****s->addr=%" PRIx64 "\n", (int) s->cle, s->ale,s->ce, s->wp, s->gnd, s->addr);
...
DB_PRINT("s->addr=%" PRIx64 " s->offset=%x s->addrlen=%d value=%x cmd=0x%" PRIx32 "\n", s->addr, s->offset,s->addrlen,value, s->cmd);
    if (s->ale) {
        ...
        s->addrlen ++;
DB_PRINT("s->addr=%" PRIx64 " shift=%x mask=%lx v=%lx s->addrlen=%d cmd=0x%" PRIx32 "\n", s->addr, shift, mask, v, s->addrlen,s->cmd);
        switch (s->addrlen) {
        case 1:
            if (s->cmd == NAND_CMD_READID || s->cmd == NAND_CMD_READPARAMPAGE) { 
                nand_command(s);
            }
            break;
...
DeviceState *nand_init(BlockBackend *blk, int manf_id, int chip_id, const char *param)
{
    DeviceState *dev;
    ...
    qdev_prop_set_string(dev, "param_page", param);
    if (blk) {
                ...
                uint64_t addr, int offset)
{
    ...
    if (s->blk) {
        if (s->mem_oob) {
            ...
            s->ioaddr = s->io + (PAGE_START(addr) & 0x1f) + offset;
        }
    } else {
...
DB_PRINT("%s: ******** s->addr=0x%" PRIx64 " offset=0x%x s->io=0x%x SECTOR_OFFSET= %" PRIx64 " s->ioaddr=0x%x PAGE(s->addr)=0x%" PRIx64 " PAGE_START(addr)=0x%" PRIx64 "\n", __func__, s->addr, offset, *s->io, SECTOR_OFFSET(s->addr), *s->ioaddr, PAGE(s->addr), PAGE_START(addr) );
